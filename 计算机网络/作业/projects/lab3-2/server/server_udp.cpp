...
void recv_Msgs(char* buffer, int& len) {
    ...
    while (1) {
        ...
        if (ret > 0 && r_temp.hasseqnum(expectedseqnum) && check_sum(curr_buf, Msg_size) == 0) {
            if (name_flag == 1 && expectedseqnum == 0 && r_temp.isLAST() || r_temp.isACK() || r_temp.isSYN())
                continue;
            printf("接受窗口size:1,接受seq:%d,check_sum=0\n", r_temp.msg.seq_number);
            if (r_temp.msg.length < MSS) {
                memcpy(buffer + seq_len * MSS, r_temp.msg.data, r_temp.msg.length);
                len += r_temp.msg.length;
            }
            else {
                memcpy(buffer + seq_len * MSS, r_temp.msg.data, MSS);
                len += MSS;
            }

            //发送ACK 对前expectedseqnum进行累计确认
            Mymsg s_temp = send_generateMsg(source_port, dest_port, 0, expectedseqnum, ACK, '0');//seq代表分组编号 ack=seq+1
            sendto(serverSocket, msg2char(s_temp), Msg_size, 0, (struct sockaddr*)&clientAddr, client_addrlen);
            printf("发送ACK,ack:%d,校验和:%d\n", s_temp.acknowledge_number, s_temp.checksum);
            expectedseqnum++;
