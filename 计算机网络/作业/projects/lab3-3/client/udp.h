...
public:
    ...
    bool hasseqnum(unsigned short expected) {
        if (msg.seq_number == expected)
            return 1;
        return 0;
    }
    bool hasnowseqnum(unsigned short rcvbase) {
        if (rcvbase <= msg.seq_number && msg.seq_number <= rcvbase + WINDOW_SIZE - 1) {
            return 1;
        }
        return 0;
    }
    bool hasprevseqnum(unsigned short rcvbase) {
        if (rcvbase - WINDOW_SIZE <= msg.seq_number && msg.seq_number <= rcvbase - 1) {
            return 1;
        }
        return 0;
    }
