...
- 时序为按序的。由于底层采取流式套接字，说明是基于TCP进行数据传输的，显然发送消息是按序发送的，按序接受。
...
- 然后主线程去发送消息。这里先从客户端读取输入的内容，封装成msg去发送。如果此条消息为"exit"，打印自动退出的提醒，然后主线程释放资源，程序关闭。
...
![人数限制_client端](fig/16.png)

![人数限制_server端](fig/19.png)
