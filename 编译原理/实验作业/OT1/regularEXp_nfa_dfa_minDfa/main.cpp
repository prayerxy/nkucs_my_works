        ...
        for (auto i = M.begin(); i != M.end(); i++) {
            ...
            for (auto w = transi_count.begin(); w != transi_count.end(); w++) {
                ...
                if (flag != '0') {
                    ...
                    //更新count 这里必须要更新，避免没有同步增加w->first
                    //因为stl的= operator是一个深拷贝，所以实际上映射的不是同一个集合
                    //直接把所有都拷贝一遍简单
                    ...
                    set<char>temp;
                    temp.insert(w->first);
                    count[w->first] = temp;//实际上创建了一个新集合与w->first对应。
...
int main() {
    ...
    expr2infix();
    cout << infix << endl;
    cout << "将中缀表达式转为后缀表达式后:" << endl;
    infix2suffix();
    cout << suffix << endl;
    int temp = suffix2nfa();
    cout << "NFA初态:" << temp << endl;
    cout << "NFA终态:" << left_right[temp] << endl;
    cout << "状态集:";
    ...
    cout << endl;
    printNFA();
    nfa2dfa(temp, left_right[temp]);//dfa设置
    cout << "DFA初态:" << D_S << endl;

    cout << "DFA终态集合:" << D_F << endl;
    cout << "状态集合:";
    /*for (auto it = class2state.begin(); it != class2state.end(); it++)
        cout << it->second << " ";
    cout << endl;*/
    cout << d_state << endl;
    printDFA();
